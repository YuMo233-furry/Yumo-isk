execute unless data entity @s Inventory[{Slot:24b}].tag.YuMoIsk run function yumo_isk:bundle/have/25
execute unless data entity @s Inventory[{Slot:24b}].tag.YuMoIsk run return 0
execute if score execute yumo.isk.bundle_stack matches 1 run return 0
execute store result score Inventory yumo.isk.bundle_stack run data get entity @s Inventory[{Slot:24b}].tag.YuMoIsk
...
execute in yumo_cmp:yumo_cmp_load run data modify block 1 10 0 Items[{Slot:0b}].tag set from entity @s Inventory[{Slot:24b}].tag
